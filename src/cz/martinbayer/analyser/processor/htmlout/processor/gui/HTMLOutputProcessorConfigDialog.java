package cz.martinbayer.analyser.processor.htmlout.processor.gui;
...
public class HTMLOutputProcessorConfigDialog extends TitleAreaDialog {

	private static Logger logger = Activator.getLogger();
	private HTMLOutputProcLogic logic;
	private HTMLOutputProcessorConfigDialogModel dialogModel;
	private Text chooseFileText;
	private Button chooseFileBtn;

	public HTMLOutputProcessorConfigDialog(Shell parentShell,
			HTMLOutputProcLogic logic,
			HTMLOutputProcessorConfigDialogModel dialogModel) {
		super(parentShell);
		this.logic = logic;
	...
	private void initBinding() {
		...
		IObservableValue model = BeanProperties.value(
				HTMLOutputProcessorConfigDialogModel.class,
				HTMLOutputProcessorConfigDialogModel.PROPERTY_SELECTED_FILE)
				.observe(dialogModel);
	...
	protected void fileSelected(String directoryPath, String selectedFile) {
		try {
			...
			logger.error(e, "Unable to create file");
		}
	}
